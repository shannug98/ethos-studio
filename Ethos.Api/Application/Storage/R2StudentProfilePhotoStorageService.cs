...
public class R2StudentProfilePhotoStorageService : IStudentProfilePhotoStorageService
{
    ...
    private readonly ILogger<R2StudentProfilePhotoStorageService> _logger;

    public R2StudentProfilePhotoStorageService(
        ICloudflareR2StorageService r2Storage,
        ILogger<R2StudentProfilePhotoStorageService> logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await _r2Storage.UploadAsync(
            ...
            file.FileName,
            file.ContentType,
            $"profile-photos/students/{userId}",
            cancellationToken);

        _logger.LogInformation("Student profile photo uploaded to R2: {ObjectKey}", result.ObjectKey);
        return result.PublicUrl;
    }

    public async Task<(Stream Stream, string ContentType)?> GetPhotoStreamAsync(
        string? storagePath,
        CancellationToken cancellationToken)
    {
        ...
        return await _r2Storage.GetObjectStreamAsync(objectKey, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Deleted student profile photo: {ObjectKey}", objectKey);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to delete student profile photo: {Path}", storagePath);
        }
    }
